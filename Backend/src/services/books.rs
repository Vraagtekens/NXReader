...
pub async fn upsert(
    ...
    sqlx::query_as::<_, Book>(
        ...
        DO UPDATE SET
            ...
            updated_at = now()
        RETURNING id, content_hash, title, author, file_name, storage_key, mime_type, file_size_bytes, created_at, updated_at
        "#,
    )
...
pub async fn upsert_uploaded(
    ...
    sqlx::query_as::<_, Book>(
        r#"
        INSERT INTO books (
            content_hash, title, author, file_name, storage_key, mime_type, file_size_bytes
        )
